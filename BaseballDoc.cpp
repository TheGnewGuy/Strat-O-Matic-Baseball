...
void CBaseballDoc::OnTeamsAddBaseTeams()
{
	...
	if (dlg.DoModal() == IDOK)
	{
		...
		teamRecord.TotalWins = dlg.m_TotalWins;
		teamRecord.TotalLosses = dlg.m_TotalLosses;
...
int CBaseballDoc::GetBatter(int teamID, CString FirstName, CString LastName)
{
	m_BatterRecord batterResult;
	char *sqlBatter;
	int rc;
	...
	rc = sqlite3_bind_text(m_stmt, 2, FirstName, strlen(FirstName), SQLITE_STATIC);
	if (rc != SQLITE_OK)
	{
		sprintf_s(buffer, sizeof(buffer), "Could not bind FirstName: %s\n", sqlite3_errmsg(m_db));
		AfxMessageBox(buffer);
	}
	rc = sqlite3_bind_text(m_stmt, 3, LastName, strlen(LastName), SQLITE_STATIC);
	if (rc != SQLITE_OK)
	{
		sprintf_s(buffer, sizeof(buffer), "Could not bind LastName: %s\n", sqlite3_errmsg(m_db));
		AfxMessageBox(buffer);
	}
...
int CBaseballDoc::GetPitcher(int teamID, CString FirstName, CString LastName)
{
	m_PitcherRecord pitcherResult;
	char *sqlPitcher;
	int rc;
	...
	return pitcherResult.PitcherID;
}
...
int CBaseballDoc::BatterAdd(m_BatterRecord BatterRecord)
{
	char *sqlBatter;
	int rc;
	CHAR buffer[100];
	...
	if (rc != SQLITE_DONE)
	{
		sprintf_s(buffer, sizeof(buffer), "Failed to Insert BATTER item: %s\n", sqlite3_errmsg(m_db));
		AfxMessageBox(buffer);
	}
...
int CBaseballDoc::BatterStatsAdd(m_BatterStatsRecord BatterStatsRecord)
{
	int rc;
	CHAR buffer[100];
	char *sqlBatterStats;
	...
	if (rc != SQLITE_DONE)
	{
		sprintf_s(buffer, sizeof(buffer), "Failed to Insert BatterStats item: %s\n", sqlite3_errmsg(m_db));
		AfxMessageBox(buffer);
	}
...
int CBaseballDoc::PitcherAdd(m_PitcherRecord PitcherRecord)
{
	int rc;
	CHAR buffer[100];
	char *sqlPitcher;
	...
	if (rc != SQLITE_DONE)
	{
		sprintf_s(buffer, sizeof(buffer), "Failed to Insert PITCHER item: %s\n", sqlite3_errmsg(m_db));
		AfxMessageBox(buffer);
	}
...
int CBaseballDoc::PitcherUpdate(m_PitcherRecord PitcherRecord)
{
	...
	if (rc != SQLITE_DONE)
	{
		sprintf_s(buffer, sizeof(buffer), "Failed to Update PITCHER item: %s\n", sqlite3_errmsg(m_db));
		AfxMessageBox(buffer);
	}
...
int CBaseballDoc::PitcherStatsAdd(m_PitcherStatsRecord PitcherStatsRecord)
{
	int rc;
	CHAR buffer[100];
	char *sqlPitcherStats;
	...
	if (rc != SQLITE_DONE)
	{
		sprintf_s(buffer, sizeof(buffer), "Failed to Insert PitcherStats item: %s\n", sqlite3_errmsg(m_db));
		AfxMessageBox(buffer);
	}
...
int CBaseballDoc::PitcherStatsUpdate(m_PitcherStatsRecord PitcherStatsRecord)
{
	...
	if (rc != SQLITE_DONE)
	{
		sprintf_s(buffer, sizeof(buffer), "Failed to Update PitcherStats item: %s\n", sqlite3_errmsg(m_db));
		AfxMessageBox(buffer);
	}
...
int CBaseballDoc::TeamInsert(m_TeamRecord TeamRecord)
{
	...
	if (rc != SQLITE_OK)
	{
		sprintf_s(buffer, sizeof(buffer), "Could not bind TotalWins: %s\n", sqlite3_errmsg(m_db));
		AfxMessageBox(buffer);
	}
	rc = sqlite3_bind_int(m_stmt, 5, TeamRecord.TotalLosses);
	if (rc != SQLITE_OK)
	{
		sprintf_s(buffer, sizeof(buffer), "Could not bind TotalLosses: %s\n", sqlite3_errmsg(m_db));
		AfxMessageBox(buffer);
	}
	rc = sqlite3_bind_int(m_stmt, 6, TeamRecord.HomeWins);
	if (rc != SQLITE_OK)
